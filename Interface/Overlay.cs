...
namespace Zyronaa.SDK.Client.Interface
{
	...
	public abstract class Overlay
	{
		...
		protected Overlay(IOverlayManager manager, string fileName)
		{
			...
			this.manager.On("zyronaa:ready", Ready);

			this.manager.Emit("zyronaa:load", new
			{
				overlay = this.Name,
				file = fileName
			});
		...
		{
			this.manager.Emit("zyronaa:visible", true);
		...
		{
			this.manager.Emit("zyronaa:visible", false);
		...
		protected virtual void Dispose(bool disposing)
		{
			...
			this.manager.Emit("zyronaa:destroy");
		}
